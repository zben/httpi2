...
unless RUBY_PLATFORM =~ /java/
  HTTPI2::Adapter.load_adapter(:curb)

  describe HTTPI2::Adapter::Curb do

    let(:adapter) { HTTPI2::Adapter::Curb.new(request) }
    ...
    describe "#request(:get)" do
      ...
      it "returns a valid HTTPI2::Response" do
    ...
    describe "#request(:custom)" do
      it "raises a NotSupportedError" do
        expect { adapter.request(:custom) }.
          to raise_error(HTTPI2::NotSupportedError, "Curb does not support custom HTTP methods")
      end
    end
