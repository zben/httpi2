...
describe HTTPI2::Adapter do
  let(:adapter) { HTTPI2::Adapter }
  ...
  describe ".register" do
    it "registers a new adapter" do
      ...
      expect(HTTPI2::Adapter::ADAPTERS[:custom]).to include(:class => klass, :deps => deps)
      expect(HTTPI2::Adapter::ADAPTER_CLASS_MAP[klass]).to be(name)
    end
  end
  ...
  describe ".load" do
    context "called with a valid adapter" do
      it "returns the adapter's name and class" do
        expect(adapter.load(:net_http)).to eq(HTTPI2::Adapter::NetHTTP)
      end
    end

    context "called with nil" do
      it "returns the default adapter's name and class" do
        expect(adapter.load(nil)).to eq(HTTPI2::Adapter::HTTPClient)
      end
    end
