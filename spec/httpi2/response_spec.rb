require "spec_helper"
require "httpi2/response"

describe HTTPI2::Response do

  context "normal" do
    let(:response) { HTTPI2::Response.new 200, {}, Fixture.xml }
    ...
    describe "#code" do
      ...
      it "always returns an Integer" do
        response = HTTPI2::Response.new "200", {}, ""
        expect(response.code).to eq(200)
      end
    end
  ...
  context "with cookies" do
    let(:response) { HTTPI2::Response.new 200, { "Set-Cookie" => "some-cookie=choc-chip; Path=/; HttpOnly" }, "" }

    describe "#cookies" do
      it "returns a list of cookies" do
        cookie = response.cookies.first
        expect(cookie).to be_a(HTTPI2::Cookie)
      end
    end
  end

  context "empty" do
    let(:response) { HTTPI2::Response.new 204, {}, nil }
  ...
  context "multipart" do
    let(:response) { HTTPI2::Response.new 200, { "Content-Type" => "multipart/related" }, "multipart" }
  ...
  context "error" do
    let(:response) { HTTPI2::Response.new 404, {}, "" }
  ...
  context "gzipped" do
    let(:response) { HTTPI2::Response.new 200, { "Content-Encoding" => "gzip" }, Fixture.gzip }
  ...
  context "DIME" do
    let(:response) { HTTPI2::Response.new 200, { "Content-Type" => "application/dime" }, Fixture.dime }
