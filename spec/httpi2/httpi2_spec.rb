...
# find out why httpi2 doesn't load these automatically. [dh, 2012-12-15]
require "excon"
...
describe HTTPI2 do
  let(:client) { HTTPI2 }
  let(:httpclient) { HTTPI2::Adapter.load(:httpclient) }
  let(:net_http) { HTTPI2::Adapter.load(:net_http) }
  let(:net_http_persistent) { HTTPI2::Adapter.load(:net_http_persistent) }

  before(:all) do
    HTTPI2::Adapter::Rack.mount('example.com', IntegrationServer::Application)
  end

  after(:all) do
    HTTPI2::Adapter::Rack.unmount('example.com')
  end

  describe ".adapter=" do
    it "sets the default adapter to use" do
      HTTPI2::Adapter.expects(:use=).with(:net_http)
  ...
  describe ".query_builder" do
    it "gets flat builder by default" do
      expect(client.query_builder).to eq(HTTPI2::QueryBuilder::Flat)
    end
    context "setter" do
      after { client.query_builder = HTTPI2::QueryBuilder::Flat }
      it "looks up for class if symbol" do
        client.query_builder = :nested
        expect(client.query_builder).to eq(HTTPI2::QueryBuilder::Nested)
      end
      ...
      it "accepts valid class" do
        client.query_builder = HTTPI2::QueryBuilder::Nested
        expect(client.query_builder).to eq(HTTPI2::QueryBuilder::Nested)
      end
    end
  end
  ...
  describe ".request" do
    let(:request) { HTTPI2::Request.new('http://example.com') }
    ...
    it 'follows redirects' do
      ...
      redirect = HTTPI2::Response.new(302, {'location' => redirect_location}, 'Moved')
      response = HTTPI2::Response.new(200, {}, 'success')
  ...
  HTTPI2::REQUEST_METHODS.each do |method|
    describe ".#{method}" do
      let(:request) { HTTPI2::Request.new("http://example.com") }
      ...
      HTTPI2::Adapter::ADAPTERS.each do |adapter, opts|
  ...
  context "(with reset)" do
    before { HTTPI2.reset_config! }

    after do
      HTTPI2.reset_config!
      HTTPI2.log = false  # disable for specs
    end

    describe ".log" do
      it "defaults to true" do
        expect(HTTPI2.log?).to be_truthy
      end
    end

    describe ".logger" do
      it "defaults to Logger writing to STDOUT" do
        expect(HTTPI2.logger).to be_a(Logger)
      end
    end

    describe ".log_level" do
      it "defaults to :debug" do
        expect(HTTPI2.log_level).to eq(:debug)
      end
    end

    describe ".log" do
      it "logs the given messages" do
        HTTPI2.log_level = :info
        HTTPI2.logger.expects(:info).with("Log this")
        HTTPI2.log "Log this"
      end
    end
  end
