require "spec_helper"
require "httpi2/auth/ssl"

describe HTTPI2::Auth::SSL do
  before(:all) do
  ...
  describe "VERIFY_MODES" do
    it "contains the supported SSL verify modes" do
      expect(HTTPI2::Auth::SSL::VERIFY_MODES).to eq([:none, :peer, :fail_if_no_peer_cert, :client_once])
    end
  end

  describe "#present?" do
    it "defaults to return false" do
      ssl = HTTPI2::Auth::SSL.new
  ...
  describe "SSL_VERSIONS" do
    it "contains the supported SSL versions" do
      expect(HTTPI2::Auth::SSL::SSL_VERSIONS).to eq(@ssl_versions)
    end
  end

  describe "#ssl_version" do
    subject { HTTPI2::Auth::SSL.new }
